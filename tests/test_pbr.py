...
sources = ['10.77.10.0/24', '10.77.11.0/24', '10.77.15.0/24', '10.77.20.0/24', '10.77.100.0/24']
network = dict(routing_mode='pbr', isp_gateway='192.0.2.1', router_wan_ip='192.0.2.110', management_prefixes=[sources[0]], protected_prefixes=['0.0.0.0/0'])
...
class Checks(unittest.TestCase):
    ...
    def test_complete_wizard(self):
        state = None
        answers = [('mode','new'), ('target',dict(name='test-router',host='198.51.100.1')),
    ...
    def test_forwarding_scope_and_preservation(self):
        ...
        self.assertIn(' permit ip 10.77.100.0 0.0.0.255 0.0.0.0 255.255.255.255',cli)
        self.assertIn(' deny ip any 10.77.10.0 0.0.0.255',cli)
        self.assertNotIn(' deny ip any 203.0.113.20 0.0.0.0',cli)
        self.assertIn(' set interface Tunnel100',cli)
        self.assertIn(' match address local 192.0.2.110',cli)
        ...
        self.assertNotIn('ip route 10.77.',cli)
    ...
    def test_static_compatibility(self):
        spec = ProvisioningSpec(isp_gateway='192.0.2.1',router_wan_ip='192.0.2.110',management_prefixes=['10.77.10.0/24'],protected_prefixes=['10.20.0.0/24'],tunnels=[tunnel])
        cli = render_nonsecret(spec)['configuration_cli_preview']
        self.assertIn('ip route 10.77.10.0 255.255.255.0 192.0.2.1',cli)
        self.assertIn('ip route 10.20.0.0 255.255.255.0 Tunnel100 1',cli)
        self.assertTrue(render_netconf(spec)['payload_complete'])
        state = WizardState(mode='template',target={'name':'test'},network=dict(isp_gateway='192.0.2.1',router_wan_ip='192.0.2.110',management_prefixes=['10.77.10.0/24'],protected_prefixes=['10.20.0.0/24']))
    ...
    def test_preserved_tunnel_rejected(self):
        state = WizardState(mode='new',target={'name':'test-router','host':'198.51.100.1'},bootstrap={'management_ready':True,'netconf_ready':True},routing={'reviewed':True,'occupied_tunnel_ids':[1],'preserve_tunnel_ids':[1]},network=network,pbr=policy)
